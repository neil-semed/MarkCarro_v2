...
async function registrarKM(dados) {
  ...
  const corpo = await _chamarKmBridge('registrar', { ...dados, nome_condutor });
  return corpo.data;
}

async function buscarKM_porData(emailCondutor, data) {
  const corpo = await _chamarKmBridge('buscar_data', { email_condutor: emailCondutor, data });
  return corpo.data;
}

async function buscarKM_porPeriodo(emailCondutor, dataIni, dataFim) {
  const corpo = await _chamarKmBridge('buscar_periodo', { email_condutor: emailCondutor, data_ini: dataIni, data_fim: dataFim });
  return corpo.data;
}
