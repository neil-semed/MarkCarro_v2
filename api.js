...
// ============================================================
// Todas as funções abaixo usam `_supabase` (window.supabase), o
...
async function criarUsuarioComoAdmin(email, senha, metaDados) {
  _checarClient();
  const { data: { session: sessaoAntes } } = await _supabase.auth.getSession();
...
async function buscarKM_porData(emailCondutor, data) {
  _checarClient();
  const { data: resultado, error } = await _supabase
    .from('registros_km')
    .select('*')
