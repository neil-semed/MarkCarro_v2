...
function voltarParaPainelGestor() {
  ...
  // por sua vez chama carregarPainelGestor() SEM forçar atualização. Se o
  // Dashboard já tinha sido aberto antes (tabela #tb-gestor-geral já com
  // linhas), carregarPainelGestor() tem uma proteção que pula o
  // recarregamento (pra não piscar a tela ao só trocar de aba) - só que
  // isso também significava não reaplicar os filtros/recontar os KPIs
  // depois de mudanças feitas em outras telas (ex.: confirmar uma corrida
  // pela Agenda). Forçando com "true" aqui garante que, especificamente ao
  // VOLTAR de outra tela, a tabela de Gerenciamento de Solicitações e os
  // KPIs sempre venham atualizados.
