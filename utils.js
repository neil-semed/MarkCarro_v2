...
function formatarDataHoraBR(valor) {
  ...
  // CORREÇÃO: timestamps do Supabase vêm em ISO 8601 com "T" no meio
  // (ex: "2026-09-14T13:45:00+00:00"), não com espaço - o split(" ")
  // antigo não achava separador nenhum, jogava a string inteira (com
  // "T" e fuso) pro formatarDataBR, e a hora saía sempre em branco.
  // new Date() entende ISO 8601 nativamente, então tentamos isso primeiro.
  const data = new Date(valor);
  if (!isNaN(data.getTime())) {
    return `${formatarDataBR(data)} ${formatarHoraBR(data)}`;
  }
  // Fallback pro formato antigo "YYYY-MM-DD HH:MM:SS" (com espaço),
  // caso algum dado venha nesse formato em vez de ISO.
...
// PEDIDO DO USUÁRIO ("aba Próximas não carrega nada"): contas antigas
// podem ter o e-mail salvo em profiles/solicitacoes com letras maiúsculas
// diferentes de como o Supabase Auth guarda internamente (ele sempre
// normaliza pra minúsculas) - uma comparação exata (===) entre
// condutor_ida/condutor_volta e usuarioAtual.email então falha só por
// causa da caixa das letras, mesmo sendo "o mesmo" e-mail, e a corrida
// some da tela sem erro nenhum. Usado em toda comparação de e-mail de
// condutor (Painel do Dia, Agenda Geral, Dashboard, Próximas Agendas).
...
//
// CORREÇÃO (pedido do usuário: "colocar som acionável toda notificação
// recebida" - o disparo já existia em atualizarContadorNotificacoes(),
// login.js, mas NUNCA se ouvia nada): todo navegador bloqueia áudio até a
// pessoa interagir com a página pelo menos uma vez (política de autoplay) -
// como o contador de notificações roda sozinho a cada 60s em segundo
// plano, criar um AudioContext NOVO a cada chamada (como era antes) sempre
// nascia "suspended" nesse meio tempo, e osc.start() numa contexto
// suspenso não toca nada nem lança erro nenhum (falha 100% silenciosa,
// sem console.error pra avisar). Agora usa UM ÚNICO AudioContext
// compartilhado, "destravado" (resume()) no primeiro clique/toque da
// pessoa em qualquer lugar da página (ver _destravarSomNotificacao logo
// abaixo) - depois disso ele fica liberado pelo resto da sessão, mesmo
// pra sons disparados sozinhos pelo polling.
let _ctxSomNotificacao = null;

function _obterCtxSomNotificacao() {
  if (_ctxSomNotificacao) return _ctxSomNotificacao;
  const Ctx = window.AudioContext || window.webkitAudioContext;
  if (!Ctx) return null;
  try {
    _ctxSomNotificacao = new Ctx();
  } catch (e) {
    return null;
  }
  return _ctxSomNotificacao;
}

function _destravarSomNotificacao() {
  const ctx = _obterCtxSomNotificacao();
  if (ctx && ctx.state === 'suspended') {
    ctx.resume().catch(() => {});
  }
}

// Primeiro clique/toque em QUALQUER lugar da página (login, qualquer tela)
// já destrava o áudio pro resto da sessão - roda uma vez só.
document.addEventListener('click', _destravarSomNotificacao, { once: true });
document.addEventListener('touchstart', _destravarSomNotificacao, { once: true, passive: true });
