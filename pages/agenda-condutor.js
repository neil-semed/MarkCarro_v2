...
// ida e de volta"): mesmo nome de _nomeCondutor(), acrescentando o telefone
// (igual ao padrão já usado em Viagens do Dia - _condutorInfoViagensDoDia).
function _nomeTelefoneCondutor(email) {
  if (!email) return '';
  const c = (typeof cacheCondutoresParaExibicao !== 'undefined' ? cacheCondutoresParaExibicao : []).find(x => x.email === email);
...
function linhasEscalacaoCondutores(s) {
  ...
  if (emailIda && emailVolta && emailIda === emailVolta) {
    return [`Ida e Volta: ${_nomeTelefoneCondutor(emailIda)}`];
  }
  const linhas = [];
  if (emailIda) linhas.push(`Ida: ${_nomeTelefoneCondutor(emailIda)}`);
  if (emailVolta) linhas.push(`Volta: ${_nomeTelefoneCondutor(emailVolta)}`);
  return linhas;
}
