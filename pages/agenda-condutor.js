...
async function carregarAgendaCondutor() {
  ...
  try {
    ...
    // PEDIDO DO USUÁRIO ("tela agenda geral - apresentar todas as agendas
    // da data selecionada, com status confirmada"): sem isso, a Agenda
    // Geral mostrava TODAS as corridas do período pra quem tem essa
    // permissão, incluindo Pendente/Em Análise (sem condutor de verdade
    // ainda) e Cancelada/Ocupado (que mantêm o condutor_ida/volta salvo
    // mesmo depois de canceladas) - o condutor via corridas que nem
    // aconteceriam, ou que já foram desmarcadas.
    dados = (dados || []).filter(s => (s.status || 'Pendente') === 'Confirmada');
