...
async function salvarCadastro() {
  ...
  try {
    ...
    if (resultado && resultado.user) {
      // CORREÇÃO ("não cria solicitante"): completarCadastroProprio() (RPC
      // completar_cadastro_proprio, SECURITY DEFINER) no lugar de
      // atualizarPerfil() (UPDATE direto) - o UPDATE direto rodava sem
      // sessão nenhuma quando o projeto exige confirmação de e-mail
      // (signUp() não devolve sessão até confirmar), e a política de RLS
      // sempre bloqueava silenciosamente, sem erro nenhum: a conta de
      // login era criada, mas telefone/Unidade/Setor nunca eram salvos.
      const dadosPerfil = {
        telefone,
        ...
        capacidade: metaDados.capacidade || null,
...
// O link "Criar conta" na tela de login chamava alternarTelas(), uma função
// que nunca existiu no app novo (era um nome do rascunho original) - o link
// não fazia nada. Adicionado o equivalente de ida (voltarLogin já cobria a
// volta).
