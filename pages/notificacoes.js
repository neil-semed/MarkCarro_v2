...
function renderizarTelaNotificacoes(notifs) {
  ...
  const btnLimpar = document.getElementById('btn-limpar-notificacoes-tela');
  if (!lista) return;
...
// PEDIDO DO USUÁRIO ("criar opção para limpar as notificações" - ao tirar
// o sino do topo, que também servia pra isso indiretamente): apaga TODAS
// as notificações da lista (não só marca como lida) - pede confirmação
// antes, já que não tem como desfazer.
...
window.limparNotificacoesTela = limparNotificacoesTela;
