...
async function enviarSolicitacao() {
  ...
  try {
    ...
    // - mas o app limpava o formulário e trocava de tela IGUAL, como se
    // tivesse dado certo). Só limpa/navega quando pelo menos 1 realmente
    // foi salva - numa falha total, o usuário fica na própria tela, com
    // os dados ainda preenchidos (não perde o que digitou) e o toast de
    // erro já mostrado acima explica o que houve.
    if (sucesso > 0) {
      prepararFormSolicitacao();
      // Quando é o Gestor/Admin criando em nome de outra pessoa
      // (Solicitante Externo), "Minhas Solicitações" é a tela ERRADA pra
      // conferir o resultado - ela só mostra solicitações com
      // email_solicitante = e-mail do próprio Admin, então a solicitação
      // recém-criada pra outra pessoa nunca aparece lá (mais um motivo
      // de "não carregou" mesmo com o INSERT funcionando). Gerenciamento
      // de Solicitações mostra todas, é onde ela realmente aparece.
      if (modoExterno) {
