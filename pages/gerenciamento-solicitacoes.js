...
// PEDIDO DO USUÁRIO ("criar possibilidade de reverter status Ocupado e
// Cancelado"): volta a solicitação pra "Em Análise" - mesmo status que
// qualquer "Pendente" já vira sozinha ao abrir esta tela (ver
// marcarPendentesComoEmAnalise) - reabrindo as ações normais (Confirmar/
// Ocupado/Cancelar). Não mexe em condutor_ida/condutor_volta: se a
// solicitação foi cancelada/ocupada depois de já ter motorista atribuído,
// o gestor continua vendo esse motorista pré-selecionado ao reverter, em
// vez de ter que escalar de novo do zero.
async function reverterStatusGestor(id) {
  const solicitacao = cacheTodasSolicitacoesGestor.find(s => String(s.id) === String(id));
  const statusAtual = solicitacao?.status || '';
  if (!confirm(`Reverter esta solicitação de "${statusAtual}" para "Em Análise"?`)) return;
  ...
  try {
    ...
    if (solicitacao) {
      ...
      criarNotificacao({
        email_destinatario: solicitacao.email_solicitante,
        tipo: 'revertido',
        mensagem: `Sua solicitação de ${quando} (${trecho}) voltou a ser analisada pela gestão.`,
        lida: false
      }).catch(e => console.warn('Erro ao notificar solicitante:', e));
    }
    ...
    console.error('Erro ao reverter status da solicitação', id, e);
