...
function handleSelectLocalGestor(selectEl, id, campo) {
  ...
  salvarCampoGestor(id, campo, selectEl.value);
}
...
function renderizarTabelaGestorCompleta(dados) {
  ...
  tbody.innerHTML = dados.map(s => {
    ...
    <tr data-id="${s.id}">
      ...
      <td><input type="date" value="${s.data_viagem || ''}" class="input-field text-sm py-1.5 px-2" onchange="salvarCampoGestor('${s.id}', 'data_viagem', this.value)"></td>
      ...
      <td style="min-width:240px">
        ...
        <p class="text-xs text-slate-600 whitespace-normal break-words mb-1">${escGestor(s.origem) || '—'}</p>
        <select class="input-field text-sm py-2 px-2 w-full" onchange="handleSelectLocalGestor(this, '${s.id}', 'origem')">
          ${gerarOpcoesLocaisGestor(s.origem)}
        </select>
        <input type="text" value="${escGestor(s.origem)}" placeholder="Digite o local" class="input-field text-sm py-1.5 px-2 mt-1 hidden" onchange="salvarCampoGestor('${s.id}', 'origem', this.value)">
      </td>
      <td style="min-width:240px">
        <p class="text-xs text-slate-600 whitespace-normal break-words mb-1">${escGestor(s.destino) || '—'}</p>
        <select class="input-field text-sm py-2 px-2 w-full" onchange="handleSelectLocalGestor(this, '${s.id}', 'destino')">
          ${gerarOpcoesLocaisGestor(s.destino)}
        </select>
        <input type="text" value="${escGestor(s.destino)}" placeholder="Digite o local" class="input-field text-sm py-1.5 px-2 mt-1 hidden" onchange="salvarCampoGestor('${s.id}', 'destino', this.value)">
      </td>
      ...
      <!-- PEDIDO DO USUÁRIO: dar permissão pra editar a Justificativa aqui
           ...
           padrão inline-editável das outras colunas desta tabela
           (salvarCampoGestor grava direto ao sair do campo). -->
      <td style="min-width:160px">
        <textarea rows="2" class="input-field text-sm py-1.5 px-2" style="min-width:150px" onchange="salvarCampoGestor('${s.id}', 'justificativa', this.value)">${escGestor(s.justificativa)}</textarea>
      </td>
      <td>${s.tipo_viagem}</td>
      <td style="min-width:80px"><input type="number" value="${s.qtd_pessoas}" class="input-field text-sm py-1.5 px-2" min="1" onchange="salvarCampoGestor('${s.id}', 'qtd_pessoas', this.value)"></td>
      <td><span class="badge ${classeStatus(status)}">${status}</span></td>
      <td>
        <select class="input-field text-sm py-1.5 px-2 select-condutor-ida" onchange="salvarCampoGestor('${s.id}', 'condutor_ida', this.value)">
          <option value="">—</option>
          ${opcoesCondutor}
        </select>
      </td>
      <td>
        <select class="input-field text-sm py-1.5 px-2 select-condutor-volta" onchange="salvarCampoGestor('${s.id}', 'condutor_volta', this.value)">
          <option value="">—</option>
          ${opcoesCondutor}
        </select>
      </td>
      <td>
        ...
        <div class="flex flex-wrap gap-0.5">
          ...
          ` : status === 'Confirmada' ? `
            ...
            <button class="btn-danger text-[10px] leading-tight py-0.5 px-1" onclick="cancelarSolicitacaoGestor('${s.id}')">Cancelar</button>
          ` : ''}
          <button class="btn-outline text-[10px] leading-tight py-0.5 px-1" onclick="editarSolicitacaoGestor('${s.id}')">Editar</button>
        </div>
      </td>
    </tr>
    `;
  ...
  dados.forEach(s => {
    ...
    if (row) {
      ...
      if (selIda) selIda.value = s.condutor_ida || '';
      if (selVolta) selVolta.value = s.condutor_volta || '';
    }
  });
}
