...
async function salvarCondutorGestor() {
  ...
  try {
    ...
    if (emailOriginal) {
      ...
      // diferente): em vez de esperar o gatilho do banco criar a linha em
      // profiles e completar por e-mail depois (frágil - dependia de
      // timing e do DEFAULT da coluna "ativo"), usa o id do usuário recém
      // criado direto e chama admin_upsert_perfil() (RPC no banco, ver
      // supabase_fix_criar_usuario_rpc.sql), que grava o perfil inteiro
      ...
      // (adminConfirmarEmail) e supabase_fix_signup_definitivo.sql. Nunca
      // bloqueia o cadastro: a conta já foi criada acima com sucesso: se
      // "Confirm email" já estiver desligado no projeto Supabase isso é só
      // um no-op, e um erro aqui não desfaz nada do que já foi salvo.
