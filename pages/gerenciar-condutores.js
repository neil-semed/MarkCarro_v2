...
async function salvarCondutorGestor() {
  ...
  const dados = {
    ...
    // PEDIDO DO USUÁRIO ("aba Próximas não carrega nada"): normaliza pra
    // minúsculas aqui na origem - o Supabase Auth já guarda o e-mail de
    // login sempre em minúsculas por conta própria, então se o admin
    // digitar com maiúscula aqui o profiles.email/condutor_ida salvo fica
    // com uma caixa DIFERENTE da que o RLS compara (auth.email()), e a
    // corrida desse condutor simplesmente some das telas dele sem erro
    // nenhum (RLS bloqueia antes de chegar no filtro do JS).
    email: document.getElementById('cond-email').value.trim().toLowerCase(),
  ...
  try {
    ...
    if (emailOriginal) {
      ...
      // CORREÇÃO (bug "super grave" persistente: "continua sem salvar novo
      // condutor na base"): listarCondutores() (api.js) só traz quem tem
      // "ativo = true" - e o gatilho handle_new_user() do banco, ao criar a
      // linha em profiles, NUNCA define essa coluna explicitamente (só
      // insere id/tipo/nome/email), contando com o DEFAULT true da coluna.
      // Se esse DEFAULT não existir de verdade no banco em produção (banco
      // criado/alterado antes desse DEFAULT ter sido definido no schema, ou
      // qualquer outra divergência entre o schema deste repositório e o
      // banco real), a linha nasce com "ativo" NULO - e null <> true faz
      // listarCondutores() nunca trazer esse condutor de volta: o cadastro
      // até funciona (a conta e o perfil são criados de verdade), mas o
      // condutor simplesmente não aparece na lista depois, dando a
      // impressão de "não salvou nada". Setando "ativo: true" aqui, no
      // próprio UPDATE que já fazemos logo após criar a conta, resolve
      // isso de vez, sem depender do banco ter o DEFAULT certo.
      await atualizarPerfilPorEmailComVerificacao(dados.email, Object.assign({ ativo: true }, perfil));
    }
    ...
    // CORREÇÃO (bug "não salva novo condutor/solicitante", 4ª tentativa):
    // registra a mensagem REAL do Supabase no console - ver
    // supabase_fix_criacao_usuarios_admin.sql para a correção do gatilho
    // handle_new_user() (duas migrações antigas se sobrescrevendo, uma
    // delas tentando um UPDATE arriscado em auth.users de dentro do
    // próprio gatilho de criação).
    console.error('Erro ao salvar condutor (Gerenciar Condutores):', e);
