...
function carregarPainelPorPerfil() {
  ...
  // header-logo-condutor não começa com "btn-", então o reset genérico acima
  // não o esconde - precisa de reset próprio aqui (btn-sair-topo-solicitante,
  // btn-pane-topo-condutor e btn-sair-topo-condutor já são pegos pelo reset
  // genérico, por começarem com "btn-").
  document.getElementById('header-logo-condutor')?.classList.add('hidden');
  ...
  if (tipo === 'admin') {
    ...
    // Solicitações = Minha Agenda, mais Registrar Km, Próximas Agendas,
    // Trocar Senha e Sair).
    ...
    document.getElementById('side-proximas-agendas')?.classList.remove('hidden');
    ...
    // Topo (pedido do usuário): favicon do projeto, saudação centralizada,
    // botão de Pane Mecânica e Sair direto - mesmo padrão já usado pro
    // Solicitante.
    document.getElementById('header-logo-condutor')?.classList.remove('hidden');
    {
      const bloco = document.getElementById('bloco-saudacao');
      if (bloco) {
    ...
    document.getElementById('btn-sair-topo-condutor')?.classList.remove('hidden');
    abrirPainelDoDia();
  } else {
    // PC: barra de pílulas fixa no topo.
    ...
    // (Dashboard/Nova/Minhas/Avisos/Menu) e o botão Sair direto no topo.
    ...
    // "Menu" da barra inferior (abre o dropdown do avatar - Alterar Senha e
    // Sair) volta a aparecer: sem a gaveta lateral, é o único caminho até
    // "Alterar Senha" no celular.
    document.getElementById('bnav-menu')?.classList.remove('hidden');
    // Topo: saudação centralizada + sino + Sair direto + menu do avatar
    // (Alterar Senha).
    {
      const bloco = document.getElementById('bloco-saudacao');
      if (bloco) {
    ...
    document.getElementById('btn-sair-topo-solicitante')?.classList.remove('hidden');
