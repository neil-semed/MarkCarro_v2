...
function carregarPainelPorPerfil() {
  ...
  // de reset próprio aqui. header-titulo-marca-padrao e "Menu" da barra
  // inferior voltam a ficar visíveis por padrão (só o Solicitante esconde o
  // primeiro; só o Condutor esconde o segundo, mais abaixo).
  ...
  if (tipo === 'admin') {
    ...
    // Barra inferior (celular) - navegação principal do Condutor.
    ...
    // "Menu" da barra inferior (pedido do usuário): tirado pro Condutor -
    // "Alterar Senha"/"Sair" ficam só no menu do avatar, no topo.
    document.getElementById('bnav-menu')?.classList.add('hidden');
    // Topo: favicon do projeto (aumentado), saudação centralizada, botão de
    // Pane Mecânica e sino - sem botão de Sair direto (pedido do usuário:
    // "tire o botão sair do topo" - continua no menu do avatar).
