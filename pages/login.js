...
function carregarPainelPorPerfil() {
  ...
  // PEDIDO DO USUÁRIO (Solicitante: "centralizar os itens do topo -
  // fanicon, saudação e nome, botões"): por padrão a faixa do topo usa
  // "justify-between" (título de um lado, ações do outro) - o Solicitante
  // não tem título nem pílulas de navegação no topo (só o bloco favicon +
  // saudação + botões), então esse bloco único ficava "sobrando" encostado
  // à esquerda em vez de centralizado. Reset aqui garante "justify-between"
  // de volta pros outros perfis; o Solicitante troca pra "justify-center"
  // mais abaixo. Nunca as duas juntas (as classes do Tailwind não obedecem
  // a ordem no HTML, só a ordem no CSS gerado - manter uma de cada vez).
  document.getElementById('header-linha-principal')?.classList.remove('justify-center');
  document.getElementById('header-linha-principal')?.classList.add('justify-between');
  ...
  if (tipo === 'admin') {
    ...
    document.getElementById('btn-senha-topo')?.classList.remove('hidden');
    document.getElementById('btn-sair-topo')?.classList.remove('hidden');
    // PEDIDO DO USUÁRIO ("centralizar os itens do topo - fanicon, saudação e
    // nome, botões"): sem título nem pílulas de navegação do lado esquerdo,
    // o "justify-between" padrão deixava o bloco favicon+saudação+botões
    // encostado à esquerda em vez de no centro da faixa - troca pra
    // "justify-center" só pro Solicitante (revertido pro Admin/Condutor no
    // reset genérico, mais acima nesta função).
    document.getElementById('header-linha-principal')?.classList.remove('justify-between');
    document.getElementById('header-linha-principal')?.classList.add('justify-center');
    {
      const bloco = document.getElementById('bloco-saudacao');
      if (bloco) {
        bloco.classList.remove('text-right', 'max-w-[112px]', 'sm:max-w-none');
