...
function carregarPainelPorPerfil() {
  ...
  // Reset dos itens exclusivos do topo do Solicitante (logo, saudação
  ...
  document.getElementById('header-logo-solicitante')?.classList.add('hidden');
  {
  ...
  if (tipo === 'admin') {
    ...
    // CORREÇÃO (pedido do usuário): a aba "Menu" da barra inferior só
    // repetia o que a gaveta lateral (☰) já oferece (Trocar Senha, Sair) -
    // trocada pelo atalho de Dashboard acima; o botão Sair direto no topo
    // (ver abaixo) cobre o caso mais comum sem precisar abrir nada.
    document.getElementById('bnav-menu')?.classList.add('hidden');
    // Topo: logo + saudação centralizada + sino + Sair direto, no lugar do
    // menu do avatar (que só repetia Alterar Senha/Sair, também já
    // acessíveis pela gaveta lateral).
    document.getElementById('header-logo-solicitante')?.classList.remove('hidden');
    {
      const bloco = document.getElementById('bloco-saudacao');
      if (bloco) {
  ...
  // CORREÇÃO: pro Solicitante o menu do avatar foi substituído pelo botão
  // Sair direto (ver bloco "else" acima) - reativar ele aqui incondicional
  // desfazia esse "add('hidden')" toda vez, porque este trecho roda DEPOIS
  // do if/else. Só reabilita pra quem realmente usa esse menu (Admin/
  // Condutor).
  if (tipo !== 'solicitante') {
