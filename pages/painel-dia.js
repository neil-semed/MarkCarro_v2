...
async function reportarPaneMecanica(id, origem, destino, emailSolicitante) {
  if (!confirm('Confirma o registro de pane mecânica nesta viagem? O solicitante e a gestão serão avisados agora.')) return;
  ...
  try {
    ...
    await notificarTodosGestores(
      `Pane mecânica reportada por ${nomeCondutor} na viagem ${trecho}.`,
      'pane_mecanica'
    );
    Components.Toast.success('Pane registrada. Solicitante e gestão foram avisados.');
  } catch (e) {
