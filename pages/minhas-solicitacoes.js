...
// > passadas.
// PEDIDO DO USUÁRIO ("agendas passadas, ordenar da mais recente para a mais
// antiga"): só o grupo de PASSADAS inverte (mais recente primeiro) - hoje e
// futuras continuam na ordem cronológica normal (mais cedo/mais próxima
// primeiro).
  ...
  return `
    <tr>
      ...
      <td class="whitespace-nowrap">
        ...
        ${motivoEdicao ? `<div class="text-[11px] text-slate-400 mt-1 italic">${motivoEdicao}</div>` : ''}
      </td>
    </tr>
  `;
}
...
function _cardMinhasSolic(s) {
  // PEDIDO DO USUÁRIO ("informar ao usuário qualquer dessas situações"):
  // mesmo motivo de _linhaTabelaMinhasSolic, aqui pro card mobile.
  const motivoEdicao = (s.status !== 'Cancelada' && s.status !== 'Desprezado') ? _motivoNaoEditavel(s) : null;
  return `
      <div class="trip-card ${classeCorBordaViagem(s.status)}">
        ...
        ${motivoEdicao ? `<div class="trip-meta-row text-slate-400"><span class="italic text-xs">${motivoEdicao}</span></div>` : ''}
      </div>
  `;
}
...
// PEDIDO DO USUÁRIO ("colocar os botões de CANCELAR agenda - até 30 minutos
// antes da viagem"): antes só liberava Cancelar pra Pendente/Em Análise -
// agora vale pra qualquer solicitação ainda "viva" (mesmo Confirmada/
// Ocupada), só pela regra de tempo. Já decidida negativamente (Cancelada
// pelo gestor, ou já Desprezada por um cancelamento anterior) não cancela de
// novo. Sem data/hora de saída definidas, libera o cancelamento (não há
// como calcular o prazo).
function podeCancelarSolicitacao(s) {
  if (s.status === 'Cancelada' || s.status === 'Desprezado') return false;
...
// PEDIDO DO USUÁRIO ("botão EDITAR - até 12h da viagem, SE NÃO houver
// motorista atribuído. Se tem motorista atribuído (salvo), mesmo que não
// confirmado, NÃO consegue alterar - informar ao usuário qualquer dessas
// situações"): substitui a regra antiga (só Pendente/Em Análise + 24h) por
// duas condições novas, nesta ordem de prioridade:
//   1) condutor_ida OU condutor_volta já preenchido no banco (SALVO, mesmo
//      que a solicitação ainda não esteja "Confirmada") -> NUNCA edita,
//      não importa quanto tempo falte.
//   2) sem motorista nenhum atribuído -> edita até faltar 12h pro horário
//      de saída (ex.: viagem 25/09 8h -> editável até 24/09 20h).
// Retorna o MOTIVO (string) de por que não pode editar, ou null se pode -
// usado tanto pra decidir se mostra o botão "Editar" quanto pra avisar o
// usuário (nos cards/tabela e nas mensagens de erro), como pedido.
function _motivoNaoEditavel(s) {
  if (s.condutor_ida || s.condutor_volta) {
    return 'Já há motorista atribuído a esta viagem - não é possível editar.';
  }
  if (s.data_viagem && s.hora_saida) {
    const dataHoraSaida = new Date(`${s.data_viagem}T${s.hora_saida.substring(0, 5)}:00`);
    if (!isNaN(dataHoraSaida.getTime())) {
      const DOZE_HORAS_MS = 12 * 60 * 60 * 1000;
      if ((dataHoraSaida.getTime() - Date.now()) <= DOZE_HORAS_MS) {
        return 'Faltam menos de 12h para o horário de saída - não é mais possível editar.';
      }
    }
  }
  return null;
...
// O admin não usa isso: ele já pode alterar qualquer solicitação, a
// qualquer momento, por Gerenciar Solicitações.
function podeEditarSolicitacao(s) {
  if (s.status === 'Cancelada' || s.status === 'Desprezado') return false;
  return _motivoNaoEditavel(s) === null;
}
...
async function abrirEdicaoSolicitacao(id) {
  ...
  overlay.querySelector('#btn-salvar-edicao-solic')?.addEventListener('click', async () => {
    // Reconfere o prazo/motorista na hora de salvar (o modal pode ter
    // ficado aberto um tempo, ou o gestor pode ter atribuído um motorista
    // nesse meio tempo) - mesma cautela do cancelamento.
    if (!podeEditarSolicitacao(s)) {
