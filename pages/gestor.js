...
async function carregarPainelGestor(forcarAtualizacao = false) {
  ...
  try {
    ...
    tbody.innerHTML = `<tr><td colspan="16" class="text-center text-red-500 p-4">Erro ao carregar painel. <button class="btn-outline text-xs py-1.5 px-2.5 ml-2" onclick="carregarPainelGestor(true)">Tentar de novo</button></td></tr>`;
...
function renderizarTabelaGestorCompleta(dados) {
  ...
  tbody.innerHTML = dados.map(s => {
    ...
    <tr data-id="${s.id}">
      ...
      <td>
        <!-- Era "btn-group btn-group-sm" + "btn btn-x" (Bootstrap, não existe
             neste app) - toda esta coluna de ações aparecia sem estilo
             nenhum, botões colados um no outro. Trocado pelas classes reais
             (.btn-success/.btn-danger/.btn-warning/.btn-outline) num flex
             container com espaçamento, igual às outras telas de gestão. -->
