...
let cacheDashCondKm = [];
const _chartsDashCond = { status: null, km: null, mes: null };
...
function _renderizarHojeEProximasDashCond() {
  ...
  // PEDIDO DO USUÁRIO ("crie um card para acumulado de viagens - carrega
  // somente do motorista logado"): total histórico de viagens confirmadas
  // deste condutor - independente do filtro de período do topo (mesmo
  // espírito do "Acumulado da Semana", só que sem limite de data).
  const acumulado = cacheDashCondViagens.filter(aprovada);
  const elAcumulado = document.getElementById('dashcond-acumulado');
  if (elAcumulado) elAcumulado.textContent = acumulado.length;
...
function aplicarFiltroDashboardCondutor() {
  ...
  _renderizarGraficoStatusDashCond();
  _renderizarGraficoKmDashCond(kmPeriodo);
  _renderizarGraficoMesDashCond(viagensPeriodo);
}

// PEDIDO DO USUÁRIO ("o gráfico de viagens por mês deve apresentar o top 7
// destinos mais frequentes do período"): trocou de "contagem por mês" pra
// "contagem por destino" - continua respeitando o período selecionado no
// topo (dias/viagens já vêm filtrados por quem chama esta função), só que
// agora mostra os 7 destinos mais frequentes, do mais pro menos frequente.
function _renderizarGraficoMesDashCond(viagens) {
  ...
  const porDestino = {};
  viagens.forEach(s => {
    const chave = s.destino || 'Sem destino';
    porDestino[chave] = (porDestino[chave] || 0) + 1;
  });
  const top7 = Object.entries(porDestino).sort((a, b) => b[1] - a[1]).slice(0, 7);

  if (_chartsDashCond.mes) { _chartsDashCond.mes.destroy(); _chartsDashCond.mes = null; }
  if (!top7.length) return;

  const paleta = ['#044AAA', '#46BE6B', '#D85736', '#FF914D', '#1D5C63', '#FFDE59', '#8854D0'];
  _chartsDashCond.mes = new Chart(canvas.getContext('2d'), {
    type: 'bar',
    data: {
      labels: top7.map(e => e[0]),
      datasets: [{ label: 'Viagens', data: top7.map(e => e[1]), backgroundColor: top7.map((_, i) => paleta[i % paleta.length]) }]
    },
    options: {
      indexAxis: 'y',
      responsive: true,
...
// PEDIDO DO USUÁRIO ("o gráfico minhas corridas - comparativo mês a mês,
// independente de filtro"): sempre usa o HISTÓRICO INTEIRO deste condutor
// (cacheDashCondViagens, sem filtrar por período) agrupado por mês/ano de
// data_viagem - por isso não recebe mais "viagens" filtradas por quem
// chama, e o seletor de período do topo não afeta este gráfico.
function _renderizarGraficoStatusDashCond() {
  const canvas = document.getElementById('dashcond-status-canvas');
  if (!canvas || typeof Chart === 'undefined') return;

  const porMes = {};
  cacheDashCondViagens.forEach(s => {
